...
public final class TuningOpModes {
    public static final Class<?> DRIVE_CLASS = OmniDrive.class;
    ...
    public static void register(OpModeManager manager) {
        ...
        if (DRIVE_CLASS.equals(MecanumDrive.class)) {
            ...
            dvf = hardwareMap -> {
                ...
                List<Encoder> leftEncs = new ArrayList<>(), rightEncs = new ArrayList<>();
                List<Encoder> parEncs = new ArrayList<>(), perpEncs = new ArrayList<>();
                if (od.localizer instanceof OmniDrive.DriveLocalizer) {
                    OmniDrive.DriveLocalizer dl = (OmniDrive.DriveLocalizer) od.localizer;
                    // Omnibot Standard (drive-encoders)
                    leftEncs.add(dl.leftFront);
                    leftEncs.add(dl.leftBack);
                    rightEncs.add(dl.rightFront);
                    rightEncs.add(dl.rightBack);
                    /*// Omnibot ForwardPushTest (drive-encoders)
                    leftEncs.add(dl.leftBack);
                    leftEncs.add(dl.leftBack);
                    rightEncs.add(dl.rightFront);
                    rightEncs.add(dl.rightFront);*/
                    /*// Omnibot ForwardPushTest replacement of LateralPushTest (drive-encoders)
                    leftEncs.add(dl.leftFront);
                    leftEncs.add(dl.leftFront);
                    rightEncs.add(dl.rightBack);
                    rightEncs.add(dl.rightBack);*/
                    /*// Omnibot AngularRampLogger (drive-encoders)
                    leftEncs.add(dl.leftBack);
                    leftEncs.add(dl.rightBack);
                    rightEncs.add(dl.leftFront);
                    rightEncs.add(dl.rightFront);*/
                } else if (od.localizer instanceof ThreeDeadWheelLocalizer) {
                    ThreeDeadWheelLocalizer dl = (ThreeDeadWheelLocalizer) od.localizer;
                    parEncs.add(dl.par0);
                    parEncs.add(dl.par1);
                    perpEncs.add(dl.perp);
                } else if (od.localizer instanceof TwoDeadWheelLocalizer) {
                    TwoDeadWheelLocalizer dl = (TwoDeadWheelLocalizer) od.localizer;
                    parEncs.add(dl.par);
                    perpEncs.add(dl.perp);
                } else {
                    throw new IllegalArgumentException("unknown localizer: " + od.localizer.getClass().getName());
                }

                return new DriveView(
                        DriveType.OMNI,
                        OmniDrive.PARAMS.inPerTick,
                        OmniDrive.PARAMS.maxWheelVel,
                        OmniDrive.PARAMS.minProfileAccel,
                        OmniDrive.PARAMS.maxProfileAccel,
                        hardwareMap.getAll(LynxModule.class),
                        // Omnibot Standard (drive-encoders)
                        Arrays.asList(
                                od.leftFront,
                        ...
                        /*// Omnibot ForwardRampLogger (dead-wheels)
                        Arrays.asList(
                                od.leftBack,
                                od.leftBack
                        ),
                        ...
                        /*// Omnibot AngularRampLogger (dead-wheels)
                        Arrays.asList(
                                od.leftBack,
                                od.rightBack
                        ),
                        ...
                        od.imu,
                        od.voltageSensor,
                        () -> new MotorFeedforward(OmniDrive.PARAMS.kS,
                                OmniDrive.PARAMS.kV / OmniDrive.PARAMS.inPerTick,
                                OmniDrive.PARAMS.kA / OmniDrive.PARAMS.inPerTick)
                );
            };
        } else {
